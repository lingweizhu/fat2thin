...
class TAWACqG(TKLPolicyInAC):
    ...
    def compute_loss_pi(self, data):
        ...
        x = (min_Q - value) / self.tau
        tsallis_policy = self.expq_x(x)
        nonzeros = torch.where(tsallis_policy != 0)[0]
        # print(tsallis_policy.size(), torch.where(tsallis_policy == 0))
        clipped = torch.clip(tsallis_policy, self.eps, self.exp_threshold)
        pi_loss = (log_pi[nonzeros] - log_pi_beta[nonzeros] - torch.log(clipped[nonzeros])).sum() / len(log_pi)
        # print("pi", log_pi.size(), log_pi_beta.size(), min_Q.size(), value.size(), clipped.size())
        return pi_loss, ""
