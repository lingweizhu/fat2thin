...
class InACqG(InSampleAC):
    def __init__(self, cfg):
        super(InACqG, self).__init__(cfg)
        ...
        self.beh_pi_optimizer = torch.optim.Adam(list(self.beh_pi.parameters()), cfg.pi_lr)
        self.beh_slow = Student(cfg.device, cfg.state_dim, cfg.action_dim, [cfg.hidden_units] * 2,
                                cfg.action_min, cfg.action_max, df=cfg.distribution_param)
        self.beh_slow_optimizer = torch.optim.Adam(list(self.beh_slow.parameters()), cfg.pi_lr*0.1)
    ...
    def update_beta(self, data):
        ...
        beh_slow_log_probs = self.beh_slow.log_prob(states, actions)
        beh_slow_loss = -beh_slow_log_probs.mean()
        self.beh_slow_optimizer.zero_grad()
        beh_slow_loss.backward()
        self.beh_slow_optimizer.step()
        return
    ...
    def compute_loss_pi(self, data):
        ...
        actions, log_probs = self.ac.pi.rsample(states)
        min_Q, _, _ = self.get_q_value(states, actions, with_grad=False)
        with torch.no_grad():
            value = self.get_state_value(states)
            beh_log_prob = self.beh_pi.log_prob(states, actions)
            beh_slow_log_prob = self.beh_slow.log_prob(states, actions)

        nonzeros = torch.where(beh_log_prob > -6.)[0]
        pi_loss = (log_probs - beh_log_prob - ((min_Q - value) / self.tau - beh_slow_log_prob))[nonzeros].sum()/len(log_probs)
        # print("pi", log_probs.size(), min_Q.size(), value.size(), beh_log_prob.size(), beh_slow_log_prob.size())
        return pi_loss, ""
    ...
    def save(self, timestamp=''):
        ...
        params = {
            ...
            "value_net": self.value_net.state_dict(),
            "behavior_net": self.beh_pi.state_dict(),
            "behavior_slow": self.beh_slow.state_dict(),
        }
        path = os.path.join(parameters_dir, "parameter"+timestamp)
        torch.save(params, path)
